...
const CartModal = ({products, isOpen, onClose}) => {
  ...
  return (
    ...
    style={{transition:'opacity 300ms'}}>
        <div className={`fixed right-0 top-0 md:w-1/3 w-full bg-white h-full overscroll-y-auto 
                    ...
                    <div className='cart-items'>
                        {
                            products.length ===0 ? (<div> Your Cart is Empty</div>):(
                                    products.map((item, index)=>(
                                        <div key={index} className='flex flex-col md:flex-row md:items-center md:justify-between shadow-md 
                                        md:p-5 p-2 mb-4'>
